...
class UserController {
    ...
    def index() {
		GasStation.list().each {g->
			def name = "${g.district.branch.name}${g.name}"
			name = name.replace(" ", "")
			def user = User.findByUsername(name)
			def userStation = UserGasStation.countByUser(user)
			if(!userStation && user) {
				println "insert into user_gas_station values(${user.id},${g.id});"
			}
//			println user
		}
    ...
    def list(Integer max) {
		...
		def users= User.createCriteria().list(max: max, offset: offset) {
			if(username) {
				like('username', "%${username}%")
			}
		}
        [userInstanceList: users, userInstanceTotal: users?.totalCount?users.totalCount:0]
    }
