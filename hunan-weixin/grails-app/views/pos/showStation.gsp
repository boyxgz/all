		...
		<g:hiddenField name="fullSn" value="${voucher?.fullSn }"/>
			<table>
				...
				<tr>
					...
					<td>
						<g:select name="channel.id" value="${payment?.channel?.id }" from="${VoucherPaymentChannel.list() }" optionKey="id" optionValue="name"/>
					</td>
				</tr>
				<tr>
					<td align="right">兑奖金额：</td>
					<td><g:textField name="amountByVoucher" value="${payment?.amountByVoucher }"/></td>
				</tr>
				<tr>
					<td align="right">消费金额：</td>
					<td><g:textField name="totalAmount" value="${payment?.totalAmount }"/></td>
				</tr>
