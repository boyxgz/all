				...
				<g:if test="${payments}">
					<g:each in="${payments}" status="i" var="payment">
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							...
							<td>
							<g:if test="${payment.voucher instanceof DrawPrizeVoucher }">
							${payment.voucher.ticket.remark}
							</g:if>
							<g:else>&nbsp;</g:else>
					...
					<tr class="even">
						<td>总金额：</td>
						<td colspan="5"><font color="blue">${payments.sum(){it.rechargeAmount} }</font></td>
					</tr>
				</g:if>
				</tbody>
			</table>
